  ...
  this.contextMenuItems = [
    {
      ...
      icon: 'pi pi-copy',
      command: () => {
        this.cherokeeTable.exportCSV(this.options);
      }
    }
  ];

  this.listofIndians$ = this._indianDataService?.getAllCherokeeIndians()?.pipe(
    map((data: unknown) => {
...
ngAfterViewInit(): void {
  this._appService?.cherokeeInputBehaviorSubject.subscribe((x: Array<object>) => {
    ...
    if (input.trim().length === 0) {
      this.cherokeeTable?.clear();
    } else {
      this.cherokeeTable?.filterGlobal(input, selector);
    }
  });
}
  ...
  public async refreshDataGrid(event: TableLazyLoadEvent) {
    ...
    this.listofIndians$ = this._indianDataService?.getAllCherokeeIndians();
